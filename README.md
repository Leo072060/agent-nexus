...
> 先看演示：基于 ERC-8004 的 Validator 仲裁式数字交付 Escrow  
> 抖音视频地址：<https://v.douyin.com/Npht-4BhM5M/>

复制口令：

```text
5.66 复制打开抖音，看看【数字游民手册的作品】基于 ERC-8004 的 Validator 仲... https://v.douyin.com/Npht-4BhM5M/ :3pm Eho:/ 03/14 e@O.KW
```

## 快速体验

本地 demo 已围绕一个 `Market` 合约、两个 seller agent、一个 validator service 和 buyer CLI 组织：

- `xapi-tech-intel`：固定活动信息源 + DeepSeek 技术情报问答。
- `tas-diagram`：内置 TAS 分类边界数据，生成地质 TAS 图解说明。
- `agent-nexus` CLI：买家侧命令行，可创建订单、拉取交付、发起争议并提交证据。

常用入口：

```bash
agent-nexus --help
agent-nexus market active
agent-nexus orders create --help
```

本机演示环境细节见 [`display/local-demo-environment.md`](display/local-demo-environment.md)。
